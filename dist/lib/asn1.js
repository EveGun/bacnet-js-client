...
exports.decodeOctetString = exports.decodeReal = exports.decodeSigned = exports.decodeReadAccessResult = exports.encodeReadAccessResult = exports.bacappDecodeApplicationData = exports.decodeTagNumberAndValue = exports.decodeObjectId = exports.decodeIsOpeningTag = exports.decodeIsClosingTag = exports.decodeIsClosingTagNumber = exports.decodeIsOpeningTagNumber = exports.decodeIsContextTag = exports.decodeTagNumber = exports.encodeContextSigned = exports.encodeContextBitstring = exports.bacappEncodePropertyState = exports.bacappEncodeContextDeviceObjPropertyRef = exports.bacappEncodeApplicationData = exports.decodeHostNPort = exports.encodeHostNPort = exports.encodeContextBoolean = exports.encodeReadAccessSpecification = exports.encodeClosingTag = exports.encodeOpeningTag = exports.encodeContextObjectId = exports.encodeApplicationTimeUtc = exports.encodeApplicationTime = exports.encodeApplicationDateUtc = exports.encodeApplicationDate = exports.encodeBacnetDateUtc = exports.encodeBacnetDate = exports.encodeApplicationBitstring = exports.encodeApplicationSigned = exports.encodeApplicationEnumerated = exports.encodeApplicationUnsigned = exports.encodeApplicationObjectId = exports.encodeApplicationBoolean = exports.encodeApplicationOctetString = exports.encodeContextEnumerated = exports.encodeContextUnsigned = exports.encodeContextReal = exports.encodeTag = exports.encodeBacnetObjectId = exports.decodeEnumerated = exports.decodeUnsigned = exports.encodeUnsigned = exports.ZERO_DATE = exports.MAX_YEARS = exports.START_YEAR = void 0;
exports.encodeContextCharacterString = exports.encodeApplicationCharacterString = exports.decodeContextObjectId = exports.decodeContextCharacterString = exports.bacappEncodeContextTimestamp = exports.bacappEncodeTimestamp = exports.decodeRange = exports.decodeCalendarDatelist = exports.decodeScheduleEffectivePeriod = exports.decodeExceptionSchedule = exports.decodeWeeklySchedule = exports.decodeReadAccessSpecification = exports.decodeBacnetDatetime = exports.decodeApplicationTime = exports.decodeBacnetTime = exports.decodeApplicationDate = exports.decodeDate = exports.decodeBitstring = exports.decodeCharacterString = void 0;
const iconv = __importStar(require("iconv-lite"));
const debug_1 = __importDefault(require("debug"));
const enum_1 = require("./enum");
const trace = (0, debug_1.default)('bacnet:asn1:trace');
...
const getEncodingType = (encoding, decodingBuffer, decodingOffset) => {
    switch (encoding) {
        case enum_1.CharacterStringEncoding.UCS_2:
            ...
            return 'UTF-16BE';
        case enum_1.CharacterStringEncoding.UCS_4:
...
const decodeUcs4CharacterString = (buffer, offset, length) => {
    if (length <= 0)
        return '';
    ...
    for (let i = offset + startOffset; i + 3 < offset + length; i += 4) {
        const codePoint = littleEndian
            ...
            : ((buffer[i] << 24) |
                ...
                buffer[i + 3]) >>>
                0;
        const normalizedCodePoint = codePoint > 0x10ffff || (codePoint >= 0xd800 && codePoint <= 0xdfff)
            ? 0xfffd
            : codePoint;
    ...
    if (trailingBytes !== 0) {
        trace(`decodeUcs4CharacterString: ignoring ${trailingBytes} trailing byte(s)`);
    }
    return value;
};
const encodeUcs4CharacterString = (value) => {
...
const encodeBacnetDateUtc = (buffer, value) => {
    if (value === exports.ZERO_DATE) {
        buffer.buffer[buffer.offset++] = 0xff;
        buffer.buffer[buffer.offset++] = 0xff;
        buffer.buffer[buffer.offset++] = 0xff;
    ...
    if (value.getUTCFullYear() >= exports.START_YEAR) {
        buffer.buffer[buffer.offset++] = value.getUTCFullYear() - exports.START_YEAR;
    }
    else if (value.getUTCFullYear() < exports.MAX_YEARS) {
...
exports.encodeBacnetDateUtc = encodeBacnetDateUtc;
...
const encodeApplicationDateUtc = (buffer, value) => {
    ...
    (0, exports.encodeTag)(buffer, enum_1.ApplicationTag.DATE, false, 4);
    (0, exports.encodeBacnetDateUtc)(buffer, normalized);
};
exports.encodeApplicationDateUtc = encodeApplicationDateUtc;
const encodeBacnetTime = (buffer, value) => {
    ...
    buffer.buffer[buffer.offset++] = Math.min(99, Math.round(value.getMilliseconds() / 10));
};
const encodeBacnetTimeUtc = (buffer, value) => {
...
const encodeApplicationTimeUtc = (buffer, value) => {
    if (typeof value === 'number' && !Number.isFinite(value)) {
        throw new Error(`invalid timestamp: ${value}`);
    }
    const normalized = typeof value === 'number' ? new Date(value) : value;
...
exports.encodeApplicationTimeUtc = encodeApplicationTimeUtc;
...
const encodeReadAccessSpecification = (buffer, value) => {
    ...
    value.properties.forEach((p) => {
        ...
        if (p.index !== undefined && p.index !== enum_1.ASN1_ARRAY_ALL) {
...
const encodeHostNPort = (buffer, value) => {
    (0, exports.encodeOpeningTag)(buffer, 0);
    const host = value.host ?? { type: enum_1.HostAddressType.NONE };
    switch (host.type) {
        case enum_1.HostAddressType.IP_ADDRESS: {
            const address = host.address ?? [];
            (0, exports.encodeTag)(buffer, 1, true, address.length);
            encodeOctetString(buffer, address, 0, address.length);
            break;
        }
        case enum_1.HostAddressType.NAME:
            (0, exports.encodeContextCharacterString)(buffer, 2, host.name ?? '');
            break;
        default:
    ...
    (0, exports.encodeClosingTag)(buffer, 0);
    (0, exports.encodeContextUnsigned)(buffer, 1, value.port);
};
exports.encodeHostNPort = encodeHostNPort;
const decodeHostNPort = (buffer, offset, maxOffset) => {
    let len = 0;
    if (!(0, exports.decodeIsOpeningTagNumber)(buffer, offset + len, 0))
        return undefined;
    len++;
    const hostTag = (0, exports.decodeTagNumberAndValue)(buffer, offset + len);
    if (!hostTag)
        return undefined;
    let host;
    if (hostTag.tagNumber === 0) {
        len += hostTag.len;
        host = { type: enum_1.HostAddressType.NONE };
    }
    else if (hostTag.tagNumber === 1) {
        len += hostTag.len;
        const octets = (0, exports.decodeOctetString)(buffer, offset + len, maxOffset, 0, hostTag.value);
        len += octets.len;
        host = { type: enum_1.HostAddressType.IP_ADDRESS, address: octets.value };
    }
    else if (hostTag.tagNumber === 2) {
        len += hostTag.len;
        const name = (0, exports.decodeCharacterString)(buffer, offset + len, maxOffset - (offset + len), hostTag.value);
        len += name.len;
        host = { type: enum_1.HostAddressType.NAME, name: name.value };
    }
    else {
        return undefined;
    }
    if (!(0, exports.decodeIsClosingTagNumber)(buffer, offset + len, 0))
        return undefined;
    len++;
    const portTag = (0, exports.decodeTagNumberAndValue)(buffer, offset + len);
    if (!portTag || portTag.tagNumber !== 1)
        return undefined;
    len += portTag.len;
    const port = (0, exports.decodeUnsigned)(buffer, offset + len, portTag.value);
    len += port.len;
...
const pickIndexedWeeklyDay = (days, arrayIndex) => {
    if (days.length === 0)
        return undefined;
...
const decodeReadAccessResult = (buffer, offset, apduLen) => {
    ...
    while (apduLen - len > 0) {
        ...
        if (value.objectId.type === enum_1.ObjectType.SCHEDULE &&
            newEntry.id === enum_1.PropertyIdentifier.EFFECTIVE_PERIOD &&
            newEntry.index !== enum_1.ASN1_ARRAY_ALL) {
            return undefined;
        }
        if (value.objectId.type === enum_1.ObjectType.CALENDAR &&
            newEntry.id === enum_1.PropertyIdentifier.DATE_LIST &&
            newEntry.index !== enum_1.ASN1_ARRAY_ALL) {
            return undefined;
        }
        ...
        if (tagResult3.tagNumber === 4) {
            ...
            if (value.objectId.type === enum_1.ObjectType.SCHEDULE &&
                newEntry.id === enum_1.PropertyIdentifier.WEEKLY_SCHEDULE &&
                newEntry.index === enum_1.ASN1_ARRAY_ALL) {
                const decodedWeekly = (0, exports.decodeWeeklySchedule)(buffer, offset + len, apduLen - len, 4);
                if (!decodedWeekly)
                    return undefined;
                newEntry.value = [
                    {
            ...
            else if (value.objectId.type === enum_1.ObjectType.SCHEDULE &&
                newEntry.id === enum_1.PropertyIdentifier.WEEKLY_SCHEDULE &&
                newEntry.index !== enum_1.ASN1_ARRAY_ALL &&
                newEntry.index !== 0) {
                const decodedWeekly = (0, exports.decodeWeeklySchedule)(buffer, offset + len, apduLen - len, 4);
                ...
                const selectedWeekly = pickIndexedWeeklyDay(decodedWeekly.value, newEntry.index);
                if (!Array.isArray(selectedWeekly))
                    return undefined;
                newEntry.value = [
                    {
                        type: enum_1.ApplicationTag.WEEKLY_SCHEDULE,
                ...
                newEntry.id === enum_1.PropertyIdentifier.EXCEPTION_SCHEDULE &&
                newEntry.index === enum_1.ASN1_ARRAY_ALL) {
                const decodedException = (0, exports.decodeExceptionSchedule)(buffer, offset + len, apduLen - len, 4);
                if (!decodedException)
                    return undefined;
                newEntry.value = [
                    {
                ...
                const exceptionIdx = decodedException.value.length === 1 ? 0 : newEntry.index - 1;
                ...
                newEntry.id === enum_1.PropertyIdentifier.EFFECTIVE_PERIOD &&
                newEntry.index === enum_1.ASN1_ARRAY_ALL) {
                const decodedEffective = (0, exports.decodeScheduleEffectivePeriod)(buffer, offset + len, apduLen - len, 4, 4);
                if (!decodedEffective)
                    return undefined;
                newEntry.value = [
                    {
            ...
            else if (value.objectId.type === enum_1.ObjectType.CALENDAR &&
                newEntry.id === enum_1.PropertyIdentifier.DATE_LIST &&
                newEntry.index === enum_1.ASN1_ARRAY_ALL) {
                const decodedDateList = (0, exports.decodeCalendarDatelist)(buffer, offset + len, apduLen - len, 4, 4);
                if (!decodedDateList)
                    return undefined;
                newEntry.value = [
                    {
...
const multiCharsetCharacterstringDecode = (buffer, offset, maxLength, encoding, length) => {
    if (encoding === enum_1.CharacterStringEncoding.UCS_4) {
        return {
...
exports.decodeBacnetDatetime = decodeBacnetDatetime;
...
const decodeWeeklySchedule = (buffer, offset, apduLen, closingTagNumber = 3) => {
    ...
    while (len < apduLen &&
        !(0, exports.decodeIsClosingTagNumber)(buffer, offset + len, closingTagNumber)) {
        if (day > 6)
            return undefined;
        if ((0, exports.decodeIsOpeningTagNumber)(buffer, offset + len, 0)) {
    ...
    if (len >= apduLen)
        return undefined;
    if (!(0, exports.decodeIsClosingTagNumber)(buffer, offset + len, closingTagNumber))
        return undefined;
    len += (0, exports.decodeTagNumberAndValue)(buffer, offset + len).len;
    return { len, value: Object.values(result) };
};
exports.decodeWeeklySchedule = decodeWeeklySchedule;
const decodeExceptionSchedule = (buffer, offset, apduLen, closingTagNumber = 3) => {
    let len = 0;
    if ((0, exports.decodeIsClosingTagNumber)(buffer, offset, closingTagNumber)) {
    ...
    while (len < apduLen &&
        ...
        if ((0, exports.decodeIsOpeningTagNumber)(buffer, offset + len, 0)) {
            ...
            tag = (0, exports.decodeTagNumberAndValue)(buffer, periodTagOffset);
            len += tag.len;
            if ((0, exports.decodeIsOpeningTagNumber)(buffer, periodTagOffset, 1)) {
                const dates = [];
            ...
            decoded.date = {
                len: objectId.len,
                type: enum_1.ApplicationTag.OBJECTIDENTIFIER,
                value: {
        ...
        if (!(0, exports.decodeIsOpeningTagNumber)(buffer, offset + len, 2))
            return undefined;
        len += (0, exports.decodeTagNumberAndValue)(buffer, offset + len).len;
        while (len < apduLen &&
...
const decodeScheduleEffectivePeriod = (buffer, offset, apduLen, closingTagNumber = 3, openingTagNumber = 3) => {
    if (!(0, exports.decodeIsOpeningTagNumber)(buffer, offset - 1, openingTagNumber))
        return undefined;
    let len = 0;
    const result = [];
    while (len < apduLen &&
...
const decodeCalendarDatelist = (buffer, offset, apduLen, closingTagNumber = 3, openingTagNumber = 3) => {
    if (!(0, exports.decodeIsOpeningTagNumber)(buffer, offset - 1, openingTagNumber))
        return undefined;
    let len = 0;
    const result = [];
...
const decodeRange = (buffer, offset, maxOffset) => {
    ...
    while (offset + len < maxOffset &&
        ...
        let isFailure = false;
        if (tag.tagNumber === 0) {
            value = bacappDecodeData(buffer, offset + len, maxOffset, enum_1.ApplicationTag.BIT_STRING, tag.value);
            isLogStatus = true;
        }
        else if (tag.tagNumber === 1) {
            value = {
                type: enum_1.ApplicationTag.BOOLEAN,
            ...
            value = bacappDecodeData(buffer, offset + len, maxOffset, enum_1.ApplicationTag.UNSIGNED_INTEGER, tag.value);
        }
        else if (tag.tagNumber === 5) {
            value = bacappDecodeData(buffer, offset + len, maxOffset, enum_1.ApplicationTag.SIGNED_INTEGER, tag.value);
        }
        else if (tag.tagNumber === 6) {
            value = bacappDecodeData(buffer, offset + len, maxOffset, enum_1.ApplicationTag.BIT_STRING, tag.value);
        }
        else if (tag.tagNumber === 7) {
            value = { type: enum_1.ApplicationTag.NULL, value: null, len: 0 };
        }
        else if (tag.tagNumber === 8) {
            if (!(0, exports.decodeIsOpeningTagNumber)(buffer, datumTagOffset, 8)) {
                return undefined;
            }
            let innerLen = 0;
            const classTag = (0, exports.decodeTagNumberAndValue)(buffer, offset + len);
            innerLen += classTag.len;
            const errorClass = (0, exports.decodeEnumerated)(buffer, offset + len + innerLen, classTag.value);
            innerLen += errorClass.len;
            const codeTag = (0, exports.decodeTagNumberAndValue)(buffer, offset + len + innerLen);
            innerLen += codeTag.len;
            const errorCode = (0, exports.decodeEnumerated)(buffer, offset + len + innerLen, codeTag.value);
            innerLen += errorCode.len;
            if (!(0, exports.decodeIsClosingTagNumber)(buffer, offset + len + innerLen, 8)) {
                return undefined;
            }
            innerLen += (0, exports.decodeTagNumberAndValue)(buffer, offset + len + innerLen).len;
            value = {
                type: enum_1.ApplicationTag.ERROR,
                value: {
            ...
            if (!(0, exports.decodeIsOpeningTagNumber)(buffer, datumTagOffset, 10)) {
                return undefined;
            }
            const anyValue = (0, exports.bacappDecodeApplicationData)(buffer, offset + len, maxOffset, enum_1.ASN1_MAX_OBJECT_TYPE, enum_1.ASN1_MAX_PROPERTY_ID);
            if (!anyValue)
                return undefined;
            let innerLen = anyValue.len;
            if (!(0, exports.decodeIsClosingTagNumber)(buffer, offset + len + innerLen, 10)) {
                return undefined;
            }
            innerLen += (0, exports.decodeTagNumberAndValue)(buffer, offset + len + innerLen).len;
        ...
        len += tag.len;
        let status;
        if ((0, exports.decodeIsContextTag)(buffer, offset + len, 2)) {
            tag = (0, exports.decodeTagNumberAndValue)(buffer, offset + len);
            len += tag.len;
            status = bacappDecodeData(buffer, offset + len, maxOffset, enum_1.ApplicationTag.BIT_STRING, tag.value);
            if (!status)
                return undefined;
            len += status.len;
        }
        ...
        if (isLogStatus) {
            ...
            const logStatusBits = value.value;
            record.logStatus = {
...
const bacappDecodeContextApplicationData = (buffer, offset, maxOffset, objectType, propertyId) => {
    ...
    if (isContextSpecific(buffer[offset])) {
        ...
        if (propertyId === enum_1.PropertyIdentifier.FD_BBMD_ADDRESS) {
            const result = (0, exports.decodeHostNPort)(buffer, offset, maxOffset);
            if (!result)
                return undefined;
            return {
                type: enum_1.ApplicationTag.HOST_N_PORT,
                value: result.value,
                len: result.len,
            };
        }
        if (propertyId === enum_1.PropertyIdentifier.EVENT_TIME_STAMPS) {
            ...
            if (evtResult.tagNumber === 2) {
                subEvtResult = (0, exports.decodeBacnetDatetime)(buffer, offset + len);
                return {
                    type: enum_1.ApplicationTag.TIMESTAMP,
                    value: subEvtResult.value,
...
const encodeBacnetCharacterString = (buffer, value, encoding) => {
    ...
    const bufEncoded = encoding === enum_1.CharacterStringEncoding.UCS_4
