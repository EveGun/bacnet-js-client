...
class BACnetClient extends EventTypes_1.TypedEventEmitter {
    ...
    _processError(invokeId, buffer, offset, length) {
        ...
        this._requestManager.resolve(invokeId, new Error(`BacnetError - Class:${result.class} - Code:${result.code}`));
    }
    _processAbort(invokeId, reason) {
        this._requestManager.resolve(invokeId, new Error(`BacnetAbort - Reason:${reason}`));
    }
    ...
    _performDefaultSegmentHandling(msg, first, moreFollows, buffer, offset, length) {
        ...
        if (!moreFollows) {
            const apduBuffer = buffer_1.Buffer.concat(this._segmentStore);
            this._segmentStore = [];
    ...
    _processSegment(msg, server, buffer, offset, length) {
        ...
        if (msg.sequencenumber === 0 && this._lastSequenceNumber === 0) {
            ...
            if (msg.sequencenumber !== this._lastSequenceNumber + 1) {
                return this._segmentAckResponse(msg.header.sender, true, server, msg.invokeId, this._lastSequenceNumber, msg.proposedWindowNumber);
            }
        }
        ...
        if (msg.sequencenumber % msg.proposedWindowNumber === 0 ||
            !moreFollows) {
            this._segmentAckResponse(msg.header.sender, false, server, msg.invokeId, msg.sequencenumber, msg.proposedWindowNumber);
        }
