...
class BACnetClient extends EventTypes_1.TypedEventEmitter {
    ...
    _getSegmentAssemblyKey(msg, server) {
        const sender = this._normalizeAddress(msg.header?.sender?.address) ?? 'unknown';
    ...
    _getOutgoingSegmentKey(address, invokeId) {
    ...
    async _sendSegmentedConfirmedRequest(args, baseType) {
        ...
        const key = this._getOutgoingSegmentKey(args.receiver?.address, args.invokeId);
    ...
    _sendSegmentWindow(state) {
        ...
        if (last === state.totalSegments - 1 && !state.responseRegistered) {
            ...
            state.responsePromise = this._awaitResponse(state.invokeId, state.maxSegments);
    ...
    _findOutgoingSegmentTransaction(header, invokeId) {
        ...
        const key = this._getOutgoingSegmentKey(header?.sender?.address, invokeId);
    ...
    async _awaitResponse(invokeId, maxSegments) {
    ...
    _processAbort(invokeId, reason, header) {
    ...
    _handlePdu(buffer, offset, length, header) {
        ...
        switch (header.apduType & enum_1.PDU_TYPE_MASK) {
            ...
            case enum_1.PduType.SIMPLE_ACK:
                ...
                this._requestManager.resolve(msg.invokeId, null, {
                    msg,
                    buffer,
                    offset,
                    length,
            ...
            case enum_1.PduType.COMPLEX_ACK:
                ...
                if (!isSegmentedMessage) {
                    ...
                    const requestMaxSegments = this._getPendingRequestMaxSegments().get(msg.invokeId);
                    if (this._settings
                        ...
                        this._requestManager.resolve(msg.invokeId, new Error(`BacnetAbort - Reason:${enum_1.AbortReason.SEGMENTATION_NOT_SUPPORTED}`));
                        break;
                    }
