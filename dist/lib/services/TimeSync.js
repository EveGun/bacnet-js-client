...
class TimeSync extends AbstractServices_1.BacnetService {
    ...
    static decodeUtc(buffer, offset, _apduLen) {
        let len = 0;
        let result;
        result = baAsn1.decodeTagNumberAndValue(buffer, offset + len);
        len += result.len;
        ...
        const time = baAsn1.decodeBacnetTime(buffer, offset + len);
        len += time.len;
        if (date.value === baAsn1.ZERO_DATE ||
        ...
        return {
            len,
            value: new Date(Date.UTC(date.value.getFullYear(), date.value.getMonth(), date.value.getDate(), time.value.getHours(), time.value.getMinutes(), time.value.getSeconds(), time.value.getMilliseconds())),
        };
    }
}
