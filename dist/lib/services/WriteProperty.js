...
class WriteProperty extends AbstractServices_1.BacnetService {
    ...
    static encodeArrayLengthPayload(buffer, propertyName, values, expectedLength) {
        let length;
        if (typeof values === 'number') {
            length = values;
        }
        else if (WriteProperty.hasTypeAndValue(values) &&
            values.type === enum_1.ApplicationTag.UNSIGNED_INTEGER) {
            length = values.value;
        }
            ...
            if (values.length === 1 &&
                ...
                first.type === enum_1.ApplicationTag.UNSIGNED_INTEGER) {
                length = first.value;
            }
            else {
        ...
        if (!Number.isInteger(length) || length < 0) {
            throw new Error(`Could not encode: ${propertyName} array size must be an unsigned integer`);
        }
        if (expectedLength !== undefined && length !== expectedLength) {
            throw new Error(`Could not encode: ${propertyName} array size must be ${expectedLength}`);
        }
    ...
    static encodeWeeklyScheduleDay(buffer, day, dayLabel) {
        if (!Array.isArray(day)) {
            throw new Error(`Could not encode: ${dayLabel} should be an array`);
        }
        baAsn1.encodeOpeningTag(buffer, 0);
        for (const [slotIndex, slot] of day.entries()) {
            const timeValue = WriteProperty.normalizeTimeInput(slot?.time, `Could not encode: ${dayLabel} slot ${slotIndex}`);
            baAsn1.bacappEncodeApplicationData(buffer, {
    ...
    static encodeWeeklySchedulePayload(buffer, values, arrayIndex) {
        if (arrayIndex === 0) {
            WriteProperty.encodeArrayLengthPayload(buffer, 'weekly schedule', values, 7);
            return;
        }
        if (arrayIndex !== enum_1.ASN1_ARRAY_ALL) {
            ...
            WriteProperty.encodeWeeklyScheduleDay(buffer, values, `weekly schedule day ${arrayIndex - 1}`);
            return;
        }
        if (!Array.isArray(values)) {
            throw new Error('Could not encode: weekly schedule should be an array');
        ...
        if (!values.every((entry) => Array.isArray(entry))) {
            throw new Error('Could not encode: weekly schedule day should be an array');
        }
        for (const [index, day] of values.entries()) {
            WriteProperty.encodeWeeklyScheduleDay(buffer, day, `weekly schedule day ${index}`);
        }
    }
    static encodeExceptionDate(buffer, date) {
        ...
        if (date.type === enum_1.ApplicationTag.OBJECTIDENTIFIER) {
            baAsn1.encodeContextObjectId(buffer, 1, date.value.type, date.value.instance);
            return;
        }
        throw new Error('Could not encode: unsupported exception schedule date format');
    }
    static encodeExceptionSchedulePayload(buffer, values, arrayIndex) {
        if (arrayIndex === 0) {
            WriteProperty.encodeArrayLengthPayload(buffer, 'exception schedule', values);
            return;
        }
        ...
        if (arrayIndex === enum_1.ASN1_ARRAY_ALL) {
            ...
            if (indexedEntry == null) {
                throw new Error('Could not encode: exception schedule entry is missing for the selected index');
            }
            entries = [indexedEntry];
        }
        for (const [index, entry] of entries.entries()) {
            if (entry.date.type === enum_1.ApplicationTag.OBJECTIDENTIFIER) {
            ...
            if (!events || events.length === 0) {
                throw new Error(`Could not encode: exception schedule entry ${index} must have at least one event`);
            }
    ...
    static encodeEffectivePeriodPayload(buffer, values, arrayIndex) {
        if (arrayIndex !== enum_1.ASN1_ARRAY_ALL) {
    ...
    static encodeCalendarDateListPayload(buffer, values, arrayIndex) {
        if (arrayIndex !== enum_1.ASN1_ARRAY_ALL) {
    ...
    static encodePropertyValuePayload(buffer, objectType, propertyId, arrayIndex, values) {
        if (objectType === enum_1.ObjectType.SCHEDULE &&
            ...
            WriteProperty.encodeWeeklySchedulePayload(buffer, values, arrayIndex);
            return;
        }
        if (objectType === enum_1.ObjectType.SCHEDULE &&
            propertyId === enum_1.PropertyIdentifier.EXCEPTION_SCHEDULE) {
            WriteProperty.encodeExceptionSchedulePayload(buffer, values, arrayIndex);
            return;
        }
        if (objectType === enum_1.ObjectType.SCHEDULE &&
            propertyId === enum_1.PropertyIdentifier.EFFECTIVE_PERIOD) {
            WriteProperty.encodeEffectivePeriodPayload(buffer, values, arrayIndex);
            return;
        }
        ...
        if (!Array.isArray(values)) {
            throw new Error('Could not encode: values should be an array of BACnet application data');
        }
        ;
    ...
    static encode(buffer, objectType, objectInstance, propertyId, arrayIndex, priority, values) {
        ...
        WriteProperty.encodePropertyValuePayload(buffer, objectType, propertyId, arrayIndex, values);
