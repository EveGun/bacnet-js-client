...
test.describe('bacnet - client', () => {
	...
	test('acknowledgeAlarm should encode the caller-provided acknowledging process id', async () => {
		const client = Object.create(BACnetClient.prototype) as BACnetClient & {
			_requestManager: { add: (invokeId: number) => Promise<any> }
			_getInvokeId: () => number
			_getApduBuffer: () => { buffer: Buffer; offset: number }
		...
		client._requestManager = {
			add: async (invokeId: number) => {
				assert.strictEqual(invokeId, 77)
				return {
		...
		await client.acknowledgeAlarm(
			...
			{ type: TimeStamp.SEQUENCE_NUMBER, value: 12 },
			{ acknowledgingProcessId: 321 },
		)
		...
		const payloadOffset = 4 + npdu.len + apdu.len
		const decoded = AlarmAcknowledge.decode(
			sentRequest,
			payloadOffset,
			sentRequest.length - payloadOffset,
		)
		assert.strictEqual(decoded.acknowledgedProcessId, 321)
	})
