				...
				value: [
					...
					},
					{
						...
						raw: {
							...
							hundredths: Math.floor(time.getMilliseconds() / 10),
						},
					},
					{ type: 12, value: { type: 3, instance: 0 } },
				],
...
test.describe('WriteProperty schedule/calendar compatibility', () => {
	...
	test('should encode single weekly schedule day when array index is 7', () => {
		const buffer = utils.getBuffer()
		const sunday = [
			{
				time: {
					type: ApplicationTag.TIME,
					value: new Date(2024, 0, 7, 22, 0),
				},
				value: { type: ApplicationTag.REAL, value: 18.5 },
			},
		]
		WriteProperty.encode(
			buffer,
		...
		const result = WriteProperty.decode(buffer.buffer, 0, buffer.offset)
		assert.ok(result)
		assert.equal(result.value.property.index, 7)
	...
	test('should reject weekly schedule write with out-of-range array index', () => {
		...
		}, /weekly schedule index must be between 1 and 7/)
	})
	...
	test('should encode exception schedule array size when array index is 0', () => {
		const buffer = utils.getBuffer()
		WriteProperty.encode(
			buffer,
			ObjectType.SCHEDULE,
		...
		assert.equal(result.value.value[0].value, 3)
	})
